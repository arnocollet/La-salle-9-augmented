...
function transformationSvg(exercise){
  ...
  if(answer==="un point")return geometrySvg("Demi-tour défini par un point",`<path class="geo-line" d="M55 75 L95 45 L125 95 Z M195 115 L225 165 L265 135 Z"/><circle class="geo-point" cx="160" cy="105" r="5"/><text class="geo-accent-label" x="173" y="110">O</text>`);
...
function bisectorSvg(exercise){
  ...
  const sector=(start,end,fill,stroke)=>{const radius=38,[x1,y1]=bisectorPoint(start,radius),[x2,y2]=bisectorPoint(end,radius),large=Math.abs(end-start)>180?1:0;return `<path d="M 160 110 L ${x1} ${y1} A ${radius} ${radius} 0 ${large} 1 ${x2} ${y2} Z" fill="${fill}" fill-opacity=".22" stroke="${stroke}" stroke-width="1.8"/>`};
...
function makeBisectorQuestion(){
  if(Math.random()<.33){
    const isTrue=Math.random()<.5,first=[30,35,40][rand(0,2)],second=isTrue?first:first+[5,10][rand(0,1)];
...
function geometryQuestionSvg(exercise){
  ...
  if(exercise.bisectorTruthData)return bisectorTruthSvg(exercise);
...
function renderQuestionVisual(exercise){
  ...
  const cubeChoicesPanel=document.getElementById("cubeChoicesPanel");
  const notion=exercise.notion;
  clearActiveGeometryBoard();
  if(cubeChoicesPanel){cubeChoicesPanel.innerHTML="";cubeChoicesPanel.classList.add("hidden")}
...
{theme:"Nombres et calculs",notion:"Produit de nombres relatifs",make:()=>{let a=rand(-12,12),b=rand(-12,12);if(a===0)a=5;if(b===0)b=-3;let answer=a*b;return q(`Calcule : (${a}) × (${b})`,answer,`Le produit de deux nombres de même signe est positif ; celui de deux nombres de signes différents est négatif. (${a}) × (${b}) = ${answer}.`)}},
{theme:"Nombres et calculs",notion:"Multiplier et diviser par 10, 100, 1 000",make:()=>{let n=rand(12,999)/10,p=[10,100,1000][rand(0,2)],op=Math.random()<.5?"×":"÷",ans=op==="×"?n*p:n/p;return q(`Calcule : ${fmt(n)} ${op} ${p}`,fmt(ans),`La virgule se décale de ${String(p).length-1} rang(s).`)}},
...
function translateGeneratedText(value){
  ...
  const translated=(GENERATED_TEXT_TRANSLATIONS[lang]||[]).reduce((text,[source,target])=>text.split(source).join(target),value);
  return translateReferenceText(translated);
}
...
function translateReferenceText(value){
  const lang=window.i18n?.getLanguage?.()||"fr";
  if(lang==="fr"||!value)return value;
  let text=(REFERENCE_TRANSLATIONS[lang]||[]).reduce((result,[source,target])=>result.split(source).join(target),value);
  const words={
    en:[["d’une","of a"],["d’un","of a"],["d’","of "],["de","of"],["du","of the"],["des","of"],["et","and"],["ou","or"],["avec","with"],["sans","without"],["par","by"],["dans","in"],["sur","on"],["pour","for"],["à","to"],["au","to the"],["un","a"],["une","a"],["les","the"],["le","the"],["la","the"],["Critères","Rules"],["divisibilité","divisibility"],["Quotient","Quotient"],["reste","remainder"],["Factorisation","Factorisation"],["Produits","Products"],["liés","related"],["tables","tables"],["Multiplier","Multiplying"],["diviser","dividing"],["Addition","Addition"],["soustraction","subtraction"],["décimaux","decimals"],["entiers","whole numbers"],["Repérage","Locating"],["droite","line"],["graduée","graduated"],["Nombre","Number"],["Nombres","Numbers"],["Fractions","Fractions"],["écritures","notation"],["écriture","notation"],["égal","equal"],["égales","equal"],["opposés","opposites"],["Puissances","Powers"],["Équations","Equations"],["expression","expression"],["expressions","expressions"],["carrés","squares"],["point","point"],["points","points"],["mesures","measurements"],["Mesures","Measurements"],["Triangles","Triangles"],["particuliers","special"],["Somme","Sum"],["somme","sum"],["angle","angle"],["angles","angles"],["cercle","circle"],["probabilités","probabilities"],["situation","situation"],["problème","problem"],["pourcentage","percentage"],["séquence","sequence"],["boucle","loop"],["valeur","value"],["variable","variable"],["tableau","table"],["tableaux","tables"],["aire","area"],["Aires","Areas"],["longueur","length"],["largeur","width"],["volume","volume"],["solides","solids"],["symétrie","symmetry"],["coordonnées","coordinates"],["repère","coordinate system"],["fonction","function"],["fonctions","functions"],["linéaire","linear"],["affine","affine"],["moyenne","mean"],["médiane","median"],["série","series"],["vitesse","speed"],["Lire","Reading"],["Écrire","Writing"],["Placer","Placing"],["Reconnaître","Recognising"],["Comprendre","Understanding"],["Calculer","Calculating"],["Déterminer","Determining"],["Résoudre","Solving"],["Suivre","Following"],["Exécuter","Running"]],
    es:[["d’une","de una"],["d’un","de un"],["d’","de "],["de","de"],["du","del"],["des","de"],["et","y"],["ou","o"],["avec","con"],["sans","sin"],["par","por"],["dans","en"],["sur","sobre"],["pour","para"],["à","a"],["au","al"],["un","un"],["une","una"],["les","los"],["le","el"],["la","la"],["Critères","Criterios"],["divisibilité","divisibilidad"],["Quotient","Cociente"],["reste","resto"],["Factorisation","Factorización"],["Produits","Productos"],["liés","relacionados"],["tables","tablas"],["Multiplier","Multiplicar"],["diviser","dividir"],["Addition","Suma"],["soustraction","resta"],["décimaux","decimales"],["entiers","enteros"],["Repérage","Localización"],["droite","recta"],["graduée","graduada"],["Nombre","Número"],["Nombres","Números"],["Fractions","Fracciones"],["écritures","escrituras"],["écriture","escritura"],["égales","equivalentes"],["opposés","opuestos"],["Puissances","Potencias"],["Équations","Ecuaciones"],["expression","expresión"],["expressions","expresiones"],["carrés","cuadrados"],["point","punto"],["points","puntos"],["mesures","medidas"],["Mesures","Medidas"],["Triangles","Triángulos"],["particuliers","particulares"],["Somme","Suma"],["somme","suma"],["angle","ángulo"],["angles","ángulos"],["cercle","círculo"],["probabilités","probabilidades"],["situation","situación"],["problème","problema"],["pourcentage","porcentaje"],["séquence","secuencia"],["boucle","bucle"],["valeur","valor"],["variable","variable"],["tableau","tabla"],["tableaux","tablas"],["aire","área"],["Aires","Áreas"],["longueur","longitud"],["largeur","anchura"],["volume","volumen"],["solides","sólidos"],["symétrie","simetría"],["coordonnées","coordenadas"],["repère","referencia"],["fonction","función"],["fonctions","funciones"],["linéaire","lineal"],["affine","afín"],["moyenne","media"],["médiane","mediana"],["série","serie"],["vitesse","velocidad"],["Lire","Leer"],["Écrire","Escribir"],["Placer","Colocar"],["Reconnaître","Reconocer"],["Comprendre","Comprender"],["Calculer","Calcular"],["Déterminer","Determinar"],["Résoudre","Resolver"],["Suivre","Seguir"],["Exécuter","Ejecutar"]],
    de:[["d’une","einer"],["d’un","eines"],["d’","von "],["de","von"],["du","des"],["des","der"],["et","und"],["ou","oder"],["avec","mit"],["sans","ohne"],["par","durch"],["dans","in"],["sur","auf"],["pour","für"],["à","zu"],["au","zum"],["un","ein"],["une","eine"],["les","die"],["le","der"],["la","die"],["Critères","Regeln"],["divisibilité","Teilbarkeit"],["Quotient","Quotient"],["reste","Rest"],["Factorisation","Faktorisieren"],["Produits","Produkte"],["liés","verbunden"],["tables","Tabellen"],["Multiplier","Multiplizieren"],["diviser","Dividieren"],["Addition","Addition"],["soustraction","Subtraktion"],["décimaux","Dezimalzahlen"],["entiers","ganze Zahlen"],["Repérage","Bestimmen"],["droite","Gerade"],["graduée","Zahlengerade"],["Nombre","Zahl"],["Nombres","Zahlen"],["Fractions","Brüche"],["écritures","Schreibweisen"],["écriture","Schreibweise"],["égales","gleichwertig"],["opposés","Gegenzahlen"],["Puissances","Potenzen"],["Équations","Gleichungen"],["expression","Ausdruck"],["expressions","Ausdrücke"],["carrés","Quadrate"],["point","Punkt"],["points","Punkte"],["mesures","Maße"],["Mesures","Maße"],["Triangles","Dreiecke"],["particuliers","besondere"],["Somme","Summe"],["somme","Summe"],["angle","Winkel"],["angles","Winkel"],["cercle","Kreis"],["probabilités","Wahrscheinlichkeiten"],["situation","Situation"],["problème","Problem"],["pourcentage","Prozentsatz"],["séquence","Folge"],["boucle","Schleife"],["valeur","Wert"],["variable","Variable"],["tableau","Tabelle"],["tableaux","Tabellen"],["aire","Fläche"],["Aires","Flächen"],["longueur","Länge"],["largeur","Breite"],["volume","Volumen"],["solides","Körper"],["symétrie","Symmetrie"],["coordonnées","Koordinaten"],["repère","Koordinatensystem"],["fonction","Funktion"],["fonctions","Funktionen"],["linéaire","linear"],["affine","affin"],["moyenne","Mittelwert"],["médiane","Median"],["série","Reihe"],["vitesse","Geschwindigkeit"],["Lire","Lesen"],["Écrire","Schreiben"],["Placer","Eintragen"],["Reconnaître","Erkennen"],["Comprendre","Verstehen"],["Calculer","Berechnen"],["Déterminer","Bestimmen"],["Résoudre","Lösen"],["Suivre","Verfolgen"],["Exécuter","Ausführen"]]
  };
  const boundary=/[A-Za-zÀ-ÿ]/;
  (words[lang]||[]).forEach(([source,target])=>{
    const escaped=source.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");
    text=text.replace(new RegExp(`(^|[^A-Za-zÀ-ÿ])${escaped}(?=$|[^A-Za-zÀ-ÿ])`,"g"),`$1${target}`);
  });
  return text;
}
...
function drawCanvasSector(context,cx,cy,radius,startAngle,endAngle,fill,stroke){
  context.beginPath();context.moveTo(cx,cy);context.arc(cx,cy,radius,startAngle*Math.PI/180,endAngle*Math.PI/180,true);context.closePath();
...
function drawTransformationWorksheetVisual(context,exercise,x,y,width,color,large=false){
  ...
  if(data){
    ...
    const columns=data.columns||4,rows=data.rows||4,cell=Math.min(55,(width-30)/columns,(large?104:76)/rows),left=x+(width-cell*columns)/2,top=y+2,kind=data.kind||exercise.transformType;
    ...
    if(kind==="translation"){const [fx,fy]=center(data.from),[tx,ty]=center(data.to);context.moveTo(fx,fy);context.lineTo(tx,ty);context.stroke()}
    else if(kind==="centrale"){
      context.setLineDash([]);context.strokeStyle=color;const cx=left+columns*cell/2,cy=top+rows*cell/2;context.beginPath();context.arc(cx,cy,4,0,Math.PI*2);context.fillStyle=color;context.fill();context.stroke();context.font="700 12px Arial";context.textAlign="left";context.fillText("O",cx+7,cy+4);
    }else{
      if(data.axis==="horizontal"){context.moveTo(left-6,top+rows*cell/2);context.lineTo(left+columns*cell+4,top+rows*cell/2)}
...
function renderWorksheetPage(sheet,sheetNumber,isCorrection,options={}){
  ...
  sheet.forEach((exercise,i)=>{
    ...
    if(exercise.bisectorTruthData){
      drawBisectorTruthWorksheetVisual(context,exercise,x+32,Math.max(y+142,questionBottom+2),boxWidth-64,worksheetColor);
      if(isCorrection){
        context.fillStyle="#167333";context.font=`700 19px ${fontFamily}`;
        context.fillText(`Réponse : ${exercise.answer}`,x+32,y+229);
      }else{
